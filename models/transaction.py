...
class Transaction(Payload):
    ...
    def to_dict(self):
        return {
            ...
            "db_id": self.db_id.decode() if isinstance(self.db_id, bytes) else str(self.db_id)
        }
    ...
    def sign(self, private_key):
        self.signature = bytes(default_eccrypto.create_signature(private_key, self.get_bytes()))
        self.public_key = default_eccrypto.key_to_bin(private_key.pub())

    def verify(self):
        try:
            pk = default_eccrypto.key_from_public_bin(self.public_key)
            return pk.verify(self.signature, self.get_bytes())
